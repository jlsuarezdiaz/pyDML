...
class RCA(DML_Algorithm):
    def __init__(self,  num_dims = None, num_chunks = None):
        self.num_dims = num_dims
        self.num_chunks = num_chunks


    def _process_data(self, X,chunks):
        self.X_ = X
        self.mean_ = np.mean(X)
        self.chunks_ = chunks

        self.centrX_ = X-self.mean_ # Centrar datos
        self.N_, self.d_ = X.shape

        if self.chunks_ is None:
            if self.num_chunks is None:
                raise NotImplementedError("Currently there isn't a method to determine an arbitrary name of chunks. "
                                          "Please specify either the number of chunks or a chunk list.")
            else:
                self.chunks_ = self._obtain_chunks(X, self.num_chunks)

        else:
            if not self.num_chunks is None:
                warnings.warn("Chunks were supplied. Argument 'num_chunks' will be ignored.")

        

    def _chunk_center_data(self):
        num_chunks = self.chunks_.max() + 1
        self.chunk_mask = self.chunks_ != -1
        self.chunked_ = self.centrX_[chunk_mask,]
        chunked_labels = self.chunks_[chunk_mask]

        for c in xrange(num_chunks):
            c_mask = chunked_labels == c
            self.chunked_[c_mask,] -= self.chunked_[c_mask,].mean(axis=0)




    def _obtain_chunks(self,X,num_chunks):
    ...
    def fit(self,X, chunks):
        ...
        self._process_data(X,chunks)
        
        self._chunk_center_data()

        self.inner_cov_ = np.cov(self.chunked_,rowvar=0, bias = 1)

        if self.num_dims_ < self.d_:
            total_cov = np.cov(self.chunked_)
        return self
...
#        self.inner_cov_ = np.cov(self.centrX_, rowvar=0, bias=1)

#        # Fisher Linear Discriminant projection
#        if self.num_dims_ < self.d_:
#            total_cov = np.cov(self.centrX_[chunk_mask], rowvar=0)
#            tmp = np.linalg.lstsq(total_cov, inner_cov)[0]
#            vals, vecs = np.linalg.eig(tmp)
#            inds = np.argsort(vals)[:dim]
#            A = vecs[:, inds]
#            inner_cov = A.T.dot(inner_cov).dot(A)
#            self.L_ = _inv_sqrtm(inner_cov).dot(A.T)
#        else:
#            self.L_ = _inv_sqrtm(inner_cov).T
#
#        return self
