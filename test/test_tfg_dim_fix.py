...
# -*- coding: utf-8 -*-
"""
Created on Fri Jul 20 20:28:56 2018
...
datasets = [[('sonar',1)], [('movement_libras',1)],[('spambase',1)],[('digits',1)]]
...
for d, f in selected_dataset:
    ...
    for dim in np.unique([1,2,3,5,10,20,30,40,50,nclas-1,maxdim]):
        ...
        if selected_alg == 0:
            ...
            train_dmls = [pca,lda,anmm[3],anmm[5],anmm[7],dmlmj[3],dmlmj[5],dmlmj[7]]
        ...
        for k in [3,5,7]:
            results[d][dim]['train'][k] = pd.read_csv('./results/'+str(d)+'-dim-'+str(dim)+"-"+str(k)+'nn-train.csv',index_col=0).values
            results[d][dim]['test'][k] = pd.read_csv('./results/'+str(d)+'-dim-'+str(dim)+"-"+str(k)+'nn-test.csv',index_col=0).values
